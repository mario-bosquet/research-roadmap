...
export default function Dashboard({ total, pillarScores, roadmap, onRestart, onOpenGlossary }: Props) {
  ...
  const phasePlan = (() => {
    ...
    const focusRoadmap = roadmap.find((r) => r.pillarId === lowestPillar.id) ?? roadmap[0];
    const focusResources = focusRoadmap?.resources ?? [];
    const [primaryAsset, secondaryAsset] = focusResources;

    const deploymentAssets = (() => {
      if (primaryAsset && secondaryAsset) return `your custom ${primaryAsset} and ${secondaryAsset}`;
    ...
    const phases = [
      ...
      },
      {
        ...
        icon: ShieldCheck,
        pct: 0.2,
        description: "Lock in real-time alerting, anomaly detection, and automated executive briefings; transfer ownership to your intelligence team with a final governance audit.",
      },
    ];

    let cursor = new Date(kickoff);
    const fmtRange = (d: Date) => d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" });
    const fmtKickoff = kickoff.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
    const segments = phases.map((p) => {
      ...
      cursor = new Date(end);
      return { ...p, range: `${fmtRange(start)} - ${fmtRange(end)}`, durationLabel: `${days} DAYS` };
    });
    return { kickoffLabel: fmtKickoff, segments };
  })();

  const accentLabel = { backgroundImage: `linear-gradient(to right, ${ACCENT}, ${ACCENT_DARK})`, WebkitBackgroundClip: "text" as const, backgroundClip: "text" as const, color: "transparent" };
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-6 py-5">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded border border-slate-800 bg-slate-950/70 px-3 py-1.5 text-[11px] tracking-widest text-slate-300 transition hover:border-[#93ADFF]/40 hover:text-[#93ADFF]"
            >
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-6 py-14">
        ...
        >
          <div className="rounded-2xl border border-slate-800/90 bg-slate-950/60 p-8">
            <div className="text-[11px] tracking-[0.22em]" style={accentLabel}>MATURITY ASSESSMENT</div>
            ...
            <div className="mt-6 h-1 w-full overflow-hidden rounded-full bg-slate-900">
              <div
                ...
                style={{ width: `${((total - 30) / 120) * 100}%`, backgroundImage: `linear-gradient(to right, ${ACCENT}33, ${ACCENT})` }}
              />
            </div>
            ...
            <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <Stat icon={Clock} label="Total Implementation Hours" value={totalHours.toLocaleString()} sub="Vendor-agnostic build envelope" />
              <Stat
                icon={Gauge}
                label="Intelligence Coverage"
                value={`${efficiencyPct}%`}
                sub={efficiencyPct < 60 ? "High intelligence debt detected" : "Listening stack operating above parity"}
                emphasis={efficiencyPct < 60 ? "warn" : "ok"}
              />
              <Stat icon={Video} label="Projected Roadmap" value={timelineLabel} sub="Dedicated intelligence pod · 300h / month" />
            </div>
          </div>

          <div className="flex flex-col rounded-2xl border border-slate-800/90 bg-slate-950/60 p-6">
            <div className="text-[11px] tracking-[0.22em]" style={accentLabel}>PILLAR RADAR</div>
            <div className="mt-4 h-[560px] w-full flex-1">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={radarData} outerRadius="85%" margin={{ top: 24, right: 48, bottom: 24, left: 48 }}>
                  ...
                  <PolarAngleAxis dataKey="pillar" tick={{ fill: "#cbd5e1", fontSize: 12, letterSpacing: 1 }} />
                  <PolarRadiusAxis angle={90} domain={[0, 30]} tick={{ fill: "#475569", fontSize: 10 }} stroke="#1e293b" />
                  <Radar dataKey="full" stroke="#334155" strokeDasharray="3 3" fill="#1e293b" fillOpacity={0.15} />
                  <Radar dataKey="score" stroke={ACCENT} strokeWidth={2} fill={ACCENT} fillOpacity={0.3} activeDot={{ fill: ACCENT, stroke: ACCENT }} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <section className="mt-14">
          {roadmap.length === 0 ? (
            <div className="rounded-2xl border border-slate-800/90 bg-slate-950/60 p-10 text-center">
              <div className="text-xl font-semibold text-slate-100">Parity reached across all five pillars.</div>
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-800/90 bg-slate-950/60">
              <div className="border-b border-white/10 px-8 py-6">
                <div className="text-[11px] tracking-[0.22em]" style={accentLabel}>IMPLEMENTATION BLUEPRINT</div>
              ...
              <div className="divide-y divide-slate-800/70">
                {roadmap.map((r) => (
                  <div key={r.pillarId} className="grid gap-4 px-6 py-5 transition hover:bg-slate-900/40" style={{ gridTemplateColumns: "3fr 4fr 4fr 1fr" }}>
                    <div>
                      <div className="text-sm font-semibold text-slate-100">{r.pillarName}</div>
                      <div className="mt-1 text-[11px] tracking-widest text-slate-500">SCORE {r.score} / 30</div>
                      <div className="mt-2 h-1 w-full rounded-full bg-slate-900">
                        <div
                          className="h-full rounded-full"
                          style={{ width: `${(r.score / 30) * 100}%`, backgroundImage: `linear-gradient(to right, ${ACCENT}33, ${ACCENT})` }}
                        />
                      </div>
                    </div>
                    <div>
                        ...
                        {r.resources.map((res) => {
                          ...
                          return (
                            <button
                              ...
                              className="inline-flex items-center gap-2 rounded-full border border-slate-700/80 bg-slate-800/70 px-3 py-1 text-[12px] font-medium shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] transition-all duration-200 hover:scale-105 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-[#93ADFF]/40"
                    ...
                    <div className="text-right">
                      <div className="text-xl font-semibold tracking-[0.198em] text-gold-gradient">{r.hours.toLocaleString()}</div>
              ...
              <div className="flex items-center justify-between border-t border-slate-800/80 bg-slate-900/40 px-6 py-4">
                <div className="text-[11px] tracking-[0.22em] text-slate-400">TOTAL IMPLEMENTATION ENVELOPE</div>
          ...
          <SpotlightCard>
            <div className="p-8 md:p-10">
              <div className="flex flex-col gap-2 border-b border-slate-800/70 pb-6 md:flex-row md:items-end md:justify-between">
                <div>
                  <div className="text-[11px] tracking-[0.22em]" style={accentLabel}>START TODAY · IMPLEMENTATION TIMELINE</div>
                  <h2 className="mt-2 font-display text-2xl font-normal tracking-[0.06em] text-text-ui md:text-3xl">
                    A phased rollout, kicking off {phasePlan.kickoffLabel}.
                  </h2>
                </div>
                <div className="text-[11px] tracking-widest text-slate-500">KICKOFF · {phasePlan.kickoffLabel.toUpperCase()}</div>
              </div>

              <ol className="relative mt-10 space-y-10 pl-12">
                <span aria-hidden className="pointer-events-none absolute left-[18px] top-2 bottom-2 w-px bg-slate-800" />
                {phasePlan.segments.map((p, i) => (
                  ...
                  >
                    <div
                      className="absolute -left-12 top-0 flex h-9 w-9 items-center justify-center rounded-[12px]"
                      style={{ backgroundImage: `linear-gradient(to bottom right, ${ACCENT}, ${ACCENT_DARK})`, boxShadow: `0 0 20px -4px ${ACCENT}99` }}
                    >
                      <p.icon className="h-4 w-4 text-[#0a0c18]" strokeWidth={2} />
                    </div>
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      ...
                      <div className="text-sm font-semibold tracking-wide text-gold-gradient">{p.range}</div>
                    </div>
        ...
        <section className="mt-16 rounded-2xl border border-slate-800/90 bg-gradient-to-br from-slate-950 to-slate-900/40 p-8 md:p-10">
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
            <div className="max-w-xl">
              <div className="text-[11px] tracking-[0.22em]" style={accentLabel}>NEXT STEP</div>
            ...
            <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
              <SpotlightButton as="a" href="https://meet.keenfunnel.com/" target="_blank" rel="noopener noreferrer">
                <Video className="h-4 w-4" />
                VIDEO CALL
              </SpotlightButton>
              <SpotlightButton onClick={handleCopyShareLink} aria-live="polite" tone="sky">
                {copied ? (
...
function Stat({
  ...
  const subColor =
    emphasis === "warn" ? "text-amber-300/90" : emphasis === "ok" ? "text-emerald-300/90" : "text-slate-500";
  return (
    <div className="rounded-lg border border-slate-800/80 bg-slate-900/40 p-4">
      <Icon className="h-4 w-4 text-[#93ADFF]" />
